...
class VM(val miRepositorio: Repositorio) : ViewModel() {
    ...
    fun borraDisco(id: Int) {
        viewModelScope.launch { miRepositorio.borrarAlbum(id) }

    }

    fun modificarDisco(album: Album) {
        viewModelScope.launch { miRepositorio.modificarAlbum(album) }
    }
